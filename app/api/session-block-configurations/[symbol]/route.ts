...
export async function GET(request: Request, { params }: { params: Promise<{ symbol: string }> }) {
  try {
    ...
    const configurations = await dbMock.all('session_block_configurations');
...
export async function PATCH(request: Request, { params }: { params: Promise<{ symbol: string }> }) {
  try {
    ...
    return NextResponse.json({ message: 'Session block configuration updated', symbol, data: body });
  } catch (error) {
