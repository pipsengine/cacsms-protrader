...
export async function GET(request: Request, { params }: { params: Promise<{ symbol: string, timeframe: string }> }) {
  try {
    ...
    const filtered = states.filter((s: any) => s.symbol_id === symbolObj.id && s.timeframe === timeframe);
    
    return NextResponse.json({ data: filtered });
  } catch (error) {
    return NextResponse.json({ error: 'Failed to fetch market structure by timeframe' }, { status: 500 });
  }
}
