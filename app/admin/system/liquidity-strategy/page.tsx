...
export default function LiquidityStrategyPage() {
  ...
  const fetchData = async () => {
    ...
    try {
      ...
      const activeRes = await fetch(`/api/liquidity-strategy/setups/active?symbol=${selectedSymbol}`).then(res => res.json());
      setActiveSetups(activeRes.data || []);

      // Actually, history might be for all, but let's just fetch it
      const historyRes = await fetch(`/api/liquidity-strategy/history`).then(res => res.json());
      // Filter history for selected symbol locally to save API calls
  ...
  const handleRecalculate = async () => {
    alert(`Triggering liquidity setup detection for ${selectedSymbol}`);
  };
    ...
    <div className="p-8 max-w-[1600px] mx-auto pb-40">
      <div className="flex items-center justify-between mb-8">
        <div>
           <h1 className="text-3xl font-bold text-slate-100 flex items-center gap-3">
              ...
              Liquidity-Based Trading Engine
           </h1>
           <p className="text-slate-400 mt-2">Professional Execution Setup & Context Generation</p>
        </div>
        <div className="flex gap-3">
          ...
          >
             <RefreshCw className="w-4 h-4" /> Run Engine
          </button>
        </div>
      </div>
      ...
      {loading ? (
         <div className="grid grid-cols-1 gap-6">
           <div className="h-40 bg-[#141414] rounded-xl border border-slate-800 animate-pulse" />
         ...
         <div className="space-y-6">
            ...
            <div className="bg-[#141414] border border-slate-800 rounded-xl overflow-hidden">
               <div className="bg-[#1A1A1A] p-4 border-b border-slate-800 flex items-center gap-2">
                  ...
                  <h3 className="text-sm font-bold text-slate-200">Active High-Probability Setups</h3>
               </div>
               <table className="w-full text-left border-collapse">
                  ...
                  <tbody className="divide-y divide-slate-800/50 text-sm">
                        ...
                        <tr key={setup.id} className="hover:bg-[#1A1A1A]/30 transition-colors">
                           <td className="p-4">
                              ...
                              <div className="text-xs text-slate-500 mt-2">
                                 Sweep ID: {setup.liquidity_event_id} • Shift ID: {setup.structure_event_id} • Zone ID: {setup.zone_id}
                              </div>
                           </td>
                           <td className="p-4">
                              <div className="flex items-center gap-2 mb-1">
                                 <span className="text-lg font-bold font-mono text-slate-200">{setup.setup_score}</span>
                                 <span className="text-xs font-bold px-1.5 py-0.5 rounded bg-blue-500/20 text-blue-400 border border-blue-500/30">{setup.confidence_level}</span>
                              </div>
                           ...
                           <td className="p-4 font-mono text-xs opacity-90 leading-snug">
                              <div className="text-slate-200 mb-0.5"><span className="text-slate-500">Entry:</span> {setup.entry_price || `${setup.entry_zone_low} - ${setup.entry_zone_high}`}</div>
                              <div className="text-rose-400 mb-0.5"><span className="text-slate-500">Stop:</span> {setup.stop_loss}</div>
                     ...
                     {activeSetups.length === 0 && (
                        <tr><td colSpan={4} className="p-8 text-center text-slate-500">No active setups detected for this symbol.</td></tr>
                     )}
                  </tbody>
               </table>
            </div>
            ...
            <div className="bg-[#141414] border border-slate-800 rounded-xl overflow-hidden mt-6">
               <div className="bg-[#1A1A1A] p-4 border-b border-slate-800 flex items-center gap-2">
                  ...
                  <h3 className="text-sm font-bold text-slate-200">Historical Setups</h3>
               </div>
               <table className="w-full text-left border-collapse">
                  <thead>
                     <tr className="bg-[#0A0A0A] border-b border-slate-800/50 text-xs text-slate-500 uppercase tracking-wider">
                  ...
                  <tbody className="divide-y divide-slate-800/50 text-sm">
                     {historySetups.map(setup => (
                        <tr key={setup.id} className="hover:bg-[#1A1A1A]/30 transition-colors">
                           ...
                           <td className="p-4">
                              <span className={`text-[10px] font-bold px-2 py-0.5 rounded-full ${setup.status === 'EXECUTED' ? 'bg-emerald-500/20 text-emerald-500' : 'bg-slate-800 text-slate-400'}`}>{setup.status}</span>
                           </td>
                        </tr>
                     ))}
